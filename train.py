...
def train( agent, env,eval_env, nb_epoch,nb_cycles_per_epoch,nb_rollout_steps,nb_train_steps, warmup, output_dir, obs_norm,eval_visualize ,tb_writer = None, max_episode_length=None, pool_mode = 0 ):
    ...
    for epoch in range(nb_epoch):
        ...
        stamper.end('epoch_tail')
        stamper.end('epoch')
        with open(output_dir+'/timeline.pkl','wb') as f:
            pickle.dump(stamper.timeline,f)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--SGLD-coef', default=0.01, type=float, help='critic net learning rate')
    ...
    parser.add_argument('--pool-mode', default=3, type=int, help='agent pool mode, 0: no pool, 1: actor pool only, 2: critic pool only, 3: both actor & critic')
